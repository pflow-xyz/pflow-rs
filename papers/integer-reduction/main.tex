...
We further prove a \emph{spectral ranking preservation theorem}: incidence reduction is a diagonal approximation to eigenvector centrality of the entity co-occurrence matrix $BB^T$, and both methods produce the same ordinal ranking for all games tested.
...
\begin{itemize}
  ...
  \item A \emph{spectral ranking preservation theorem} (Theorem~\ref{thm:spectral}): incidence reduction is a diagonal approximation to eigenvector centrality of the entity co-occurrence matrix, preserving all ordinal ranking information while remaining closed-form.
...
Our analysis net construction satisfies stronger conditions---full decoupling---so we prove equilibrium properties directly (Theorem~\ref{thm:incidence-reduction}) rather than invoking the general theory.
...
Eigenvector centrality amplifies degree differences relative to incidence reduction.
...
\begin{remark}
The spectral ranking theorem positions incidence reduction as a \emph{diagonal approximation} to eigenvector centrality: both use the same co-occurrence matrix $M = BB^T$, but incidence reduction reads only $\mathrm{diag}(M) = (n_1, \ldots, n_n)$ while eigenvector centrality solves the full eigenvalue problem.
The approximation preserves ranking (ordinal equivalence) while sacrificing the finer spectral magnitudes---a favorable trade-off, since incidence reduction is closed-form ($O(1)$ per entity) while power iteration on $M$ requires $O(n^2)$ per step.
\end{remark}
