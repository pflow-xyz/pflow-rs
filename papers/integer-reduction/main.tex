...
These methods are computationally expensive, opaque, and difficult to verify---and the underlying complexity is inherent: game evaluation is PSPACE-complete for many combinatorial games (Section~\ref{sec:discussion}).
...
Write $M = D + A'$, where $D = \mathrm{diag}(n_1, \ldots, n_n)$ is the diagonal matrix of drain counts and $A'$ is the off-diagonal part of~$M$, with $A'_{ij} = M_{ij}$ for $i \neq j$ and $A'_{ii} = 0$.
We treat $A'$ as the adjacency matrix of the \emph{co-occurrence graph}: the weighted graph on the entity set~$E$ where edge $(i,j)$ has weight equal to the number of constraints containing both $e_i$ and $e_j$.
(This graph is not defined separately; it is read directly from the off-diagonal entries of $BB^T$.)
The \emph{combinatorial Laplacian} of this co-occurrence graph is $L = D - A'$, so
...
Write $M = D(I + E)$.
Note that $M$ is symmetric (since $M = BB^T$), but $E = D^{-1}A'$ is not symmetric in general because $D^{-1}$ rescales rows non-uniformly.
However, $M$ itself is a non-negative irreducible matrix, so Perron--Frobenius theory applies directly to~$M$.

The unperturbed matrix~$D$ has Perron eigenvector $\mathbf{d} = (n_1, \ldots, n_n)^T$ (any positive eigenvector of a diagonal matrix is proportional to its diagonal).
We bound the perturbation of the Perron vector of $M = D + A'$ relative to that of~$D$ using the theory of non-negative matrix perturbations (Seneta~\cite{seneta2006}, Chapter~4).
For a non-negative irreducible matrix with Perron root $\lambda_1$ and spectral gap $\gamma = \lambda_1 - |\lambda_2|$, the Perron eigenvector~$\boldsymbol{\pi}$ satisfies
\[
  \boldsymbol{\pi} = \frac{\mathbf{d}}{\|\mathbf{d}\|} + \boldsymbol{\delta}, \quad \|\boldsymbol{\delta}\|_\infty \leq \frac{\|A'\|}{\gamma}.
\]
For the ranking of components $i$ and $j$ to be preserved, we need $d_i + \delta_i > d_j + \delta_j$, i.e., $n_i - n_j > \delta_j - \delta_i$.
Since $|\delta_j - \delta_i| \leq 2\|\boldsymbol{\delta}\|_\infty$, the condition~\eqref{eq:ranking-condition} (expressed in terms of the normalized perturbation $\|E\| = \|D^{-1}A'\|$) ensures the perturbation cannot close the gap between $n_i$ and $n_j$.
...
Logarithmic scaling compresses the five-order-of-magnitude frequency range (40 to 1{,}302{,}540) into small integers; any monotonic mapping preserves the ranking, but log scaling yields manageable drain counts and value ratios.
...
\paragraph{Computational cost and complexity class separation.}
...
The low cost reflects a deeper phenomenon: incidence reduction avoids the exponential blowup inherent in game-tree evaluation by asking a fundamentally different question.
Classical game evaluation asks ``who wins from this position?''---a question that is PSPACE-complete for many combinatorial games and undecidable for unbounded ones.
Incidence reduction asks ``what is the topological importance of each position?''---a question about the \emph{structure} of the game, not the \emph{outcome} of play.

The distinction is analogous to the difference between solving an NP-hard problem and verifying a witness.
The construction of the analysis net externalizes the combinatorial structure of the game into the net topology: entities, constraints, and their incidence relationships are encoded as places, transitions, and arcs.
This encoding is where domain knowledge enters---knowing that tic-tac-toe has $2n + 2$ win lines, or that poker hands have specific combinatorial frequencies.
Once the encoding is fixed, the decoupling lemma (Lemma~\ref{lem:decoupling}) ensures that reading the structural answer is $O(1)$ per entity: the equilibrium $x_i^* = 1/n_i$ is available in closed form, with no iteration, no search, and no approximation.
The Petri net topology \emph{is} the traversal; executing it is simply reading the witness.

This complexity separation has practical consequences.
Game-tree search scales exponentially with depth and branching factor; incidence reduction scales linearly with the number of entities and constraints.
The tradeoff is expressiveness: search answers state-dependent questions (``is this particular position winning?''), while incidence reduction answers structural questions (``which positions are topologically important?'').
For games where topological importance correlates with strategic value---as demonstrated for all four game families in Section~\ref{sec:experiments}---the structural answer suffices, and the exponential cost of search is avoided entirely.
...
\newblock Springer Series in Statistics. Springer, revised printing of 2nd edition, 2006.
