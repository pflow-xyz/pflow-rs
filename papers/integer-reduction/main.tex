...
The method constructs an \emph{analysis net}---a bipartite graph of catalytic source places and drain transitions---and solves the induced mass-action ODE system to equilibrium, applying the classical chemical reaction network theory of Horn, Jackson, and Feinberg to a novel network topology.
The central structural result is a \emph{decoupling lemma}: catalytic source places hold constant concentration, causing each accumulator to obey an independent first-order ODE $\dot{x}_i = 1 - n_i x_i$, where $n_i$ is the number of drain transitions.
Because these ODEs are fully decoupled, the equilibrium $x_i^* = 1/n_i$ is available in closed form---no iterative eigenvector computation or coupled fixed-point convergence is required.
After normalization, the equilibrium concentrations yield exact rational (often integer) strategic values determined entirely by net topology.
We further prove a \emph{spectral ranking preservation theorem}: integer reduction is a diagonal approximation to eigenvector centrality of the entity co-occurrence matrix $BB^T$, and both methods produce the same ordinal ranking for all games tested.
We validate the technique on $n \times n$ tic-tac-toe grids ($n = 3, 5, 7$), recovering the classical center--corner--edge hierarchy $\{4, 3, 2\}$ in every case; on poker hand rankings, recovering the full nine-tier hierarchy from drain counts derived from combinatorial frequencies; on Connect Four ($7 \times 6$, 4-in-a-row), recovering center-column dominance with a 13:3 value ratio; and on Hex ($5 \times 5$, shortest-path win conditions), recovering anti-diagonal dominance with a 16:1 value ratio from non-Cartesian topology.
The same incidence matrix admits three complementary readings---discrete (Petri net firing rules), continuous (ODE equilibrium), and cryptographic (Groth16 ZK proofs)---with the decoupling lemma ensuring lossless round-trip between discrete and continuous semantics.
\end{abstract}
...
\begin{itemize}
  \item A formal definition of the \emph{analysis net} construction and a proof that its equilibrium concentrations are exact reciprocals of drain counts (Theorem~\ref{thm:integer-reduction}), applying the classical mass-action kinetics of chemical reaction network theory~\cite{horn1972,feinberg2019} to a novel catalytic-pump topology.
  \item A \emph{spectral ranking preservation theorem} (Theorem~\ref{thm:spectral}): integer reduction is a diagonal approximation to eigenvector centrality of the entity co-occurrence matrix, preserving all ordinal ranking information while remaining closed-form.
...
Our analysis net construction satisfies stronger conditions---full decoupling---so we prove equilibrium properties directly (Theorem~\ref{thm:integer-reduction}) rather than invoking the general theory.
...
Despite these computational differences, the two approaches produce the same ordinal ranking: Theorem~\ref{thm:spectral} establishes that integer reduction is a diagonal approximation to eigenvector centrality of the entity co-occurrence matrix, preserving all ranking information.
...
Integer reduction uses only the \emph{diagonal} of a natural matrix associated with the entity--constraint structure.
We now show that the \emph{full} spectral analysis of this matrix produces the same ranking, establishing that integer reduction captures all ordinal information of eigenvector centrality while remaining closed-form.
...
Integer reduction uses only the diagonal of~$M$: $V_i = n_i / n_{\min} = M_{ii} / \min_j M_{jj}$.
...
\begin{enumerate}
  ...
  \item[\emph{(iii)}] \textbf{(Ranking agreement.)} For $n \times n$ tic-tac-toe with $n$ odd, eigenvector centrality and integer reduction produce the same three-level ranking: center $>$ corner $>$ edge.
\end{enumerate}
\end{theorem}
...
\begin{corollary}[Spectral Amplification]\label{cor:amplification}
Eigenvector centrality amplifies degree differences relative to integer reduction.
...
\[
  \frac{\hat{\pi}_i}{\hat{\pi}_j} > \frac{\hat{V}_i}{\hat{V}_j}
  \quad\text{whenever}\quad n_i > n_j > 0 \text{ and } M \text{ has non-zero off-diagonal entries.}
\]
For $3 \times 3$ tic-tac-toe, the center-to-edge ratio is $(1 + \sqrt{13})/2 \approx 2.303$ spectrally versus $4/2 = 2$ by integer reduction.
The amplification arises because the off-diagonal entries of $M$ encode shared win-line structure: entities with high drain counts tend to share more constraints with other high-drain entities, creating mutual reinforcement in the eigenvector computation that the diagonal-only integer reduction does not capture.
\end{corollary}

\begin{remark}
The spectral ranking theorem positions integer reduction as a \emph{diagonal approximation} to eigenvector centrality: both use the same co-occurrence matrix $M = BB^T$, but integer reduction reads only $\mathrm{diag}(M) = (n_1, \ldots, n_n)$ while eigenvector centrality solves the full eigenvalue problem.
The approximation preserves ranking (ordinal equivalence) while sacrificing the finer spectral magnitudes---a favorable trade-off, since integer reduction is closed-form ($O(1)$ per entity) while power iteration on $M$ requires $O(n^2)$ per step.
\end{remark}
...
Table~\ref{tab:spectral} compares integer reduction values with eigenvector centrality of the co-occurrence matrix $M = BB^T$ (Theorem~\ref{thm:spectral}).
...
\begin{table}[ht]
\centering
\caption{Integer reduction vs.\ eigenvector centrality for $n \times n$ tic-tac-toe.  Normalized values ($\hat{V}$ and $\hat{\pi}$) set the edge class to~1.}
\label{tab:spectral}
...
The ratio $\hat{V}/\hat{\pi}$ decreases with board size because larger boards have more shared-constraint paths through high-degree cells, amplifying the spectral reinforcement effect (Corollary~\ref{cor:amplification}).
Despite this magnitude gap, the ordinal ranking is identical across all tested sizes, confirming that integer reduction captures all ranking information without solving an eigenvalue problem.
...
This fundamentally different constraint type tests integer reduction on non-Cartesian topology.
...
This illustrates how richer game topologies---non-square grids, variable-length constraints, non-Cartesian adjacency---yield more nuanced strategic assessments, all captured exactly by integer reduction.
...
In the \emph{reverse} direction, \emph{integer reduction} recovers exact integers from continuous equilibrium: the steady-state concentrations $x_i^* = 1/n_i$ are inverted and normalized to yield $V_i = n_i/n_{\min} \in \mathbb{Q}^+$.
...
The combination of integer reduction and ZK proofs enables a concrete application: a \emph{verifiable AI opponent}.
...
Theorem~\ref{thm:spectral} makes this precise: integer reduction reads only $\mathrm{diag}(BB^T)$ while eigenvector centrality reads the full co-occurrence matrix.
The off-diagonal entries (shared constraints) create spectral amplification (Corollary~\ref{cor:amplification}) but do not change the ranking for the games tested here.
...
CRNT classifies reaction networks by their deficiency and linkage structure, yielding powerful theorems about equilibrium existence and uniqueness.
...
Our contribution is not the ODE simulation itself---which is classical---but the specific \emph{analysis net construction} whose catalytic-pump topology guarantees full decoupling, yielding closed-form equilibria that bypass CRNT's general iterative methods.
...
The spectral ranking preservation theorem (Theorem~\ref{thm:spectral}) further establishes that integer reduction is a diagonal approximation to eigenvector centrality of the entity co-occurrence matrix, preserving all ordinal ranking information while remaining closed-form.
